  ...
  *)
  ...
  let function_decls : (L.llvalue * definition) StringMap.t =
    let function_decl m def = match def with 
      Def (dbinder, dexpr) -> match dbinder with
        | Some dname -> StringMap.add dname (L.define_function dname (ltype_of_expr dexpr) the_module, def) m
